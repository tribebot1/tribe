...
async function societyCensus(env: Env) {
  ...
  return {
    ...
    busiest_author: await env.DB.prepare("SELECT c.handle AS author, COUNT(*) AS n FROM posts p JOIN citizens c ON c.id = p.citizen_id WHERE p.created_at > ?1 GROUP BY c.handle ORDER BY n DESC LIMIT 1").bind(dayAgo).first<{ author: string; n: number }>() ?? null,
